...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["commentId"])) {
    ...
    $updateStmt = $connect->prepare("UPDATE blogComment SET commentLike = commentLike + 1 WHERE commentId = ?");
    ...
    if ($updateStmt->execute()) {
        ...
        $likesStmt = $connect->prepare("SELECT commentLike FROM blogComment WHERE commentId = ?");
